...
static void *unity_main(void *unused) {
    ...
    fprintf(stderr, "[flow] UnityMain: surfaceChanged 1280x720\n");
    g_unity.recreate_gfx(environment, activity, 0, surface);
...
static int merged_button(SDL_GameControllerButton button) {
    for (int slot = 0; slot < DT_MAX_PADS; ++slot)
        if (g_pads[slot] &&
            SDL_GameControllerGetButton(g_pads[slot], button))
            return 1;
    return 0;
}
...
static uint32_t semantic_buttons(void) {
    ...
    uint32_t level = 0;
    Sint16 right_trigger = merged_axis(SDL_CONTROLLER_AXIS_TRIGGERRIGHT);
    Sint16 left_trigger = merged_axis(SDL_CONTROLLER_AXIS_TRIGGERLEFT);
...
static int run_ui_loop(pthread_t unity_thread) {
    open_all_pads();
    int start_down = 0, select_down = 0;
    ...
    float cursor_x = 640.0f, cursor_y = 360.0f;
    ...
    while (!atomic_load(&g_quit)) {
        ...
        while (SDL_PollEvent(&event)) {
            ...
            if (event.type == SDL_QUIT) {
                ...
                if (cursor_active &&
                    ...
                    if (pressed && !cursor_touch_stage) {
                        inject_touch(0, cursor_x, cursor_y);
                        cursor_touch_stage = 1;
                        next_cursor_touch =
                            monotonic_milliseconds() + 35;
                        last_cursor_activity = monotonic_milliseconds();
                    }
                } else if (cursor_active &&
                           button == SDL_CONTROLLER_BUTTON_B) {
                    inject_android_key(4, pressed);
                    last_cursor_activity = monotonic_milliseconds();
                }
            }
        }
        ...
        if (any_pad() && cursor_active) {
            ...
            if (magnitude > deadzone) {
                ...
                float speed = 900.0f * response * response;
    ...
    while (!atomic_load(&g_unity_exited) &&
           monotonic_milliseconds() - shutdown_started < 10000) {
        jni_pump_ui_tasks();
        dt_audio_pump();
        SDL_Delay(4);
    }
    if (atomic_load(&g_unity_exited))
        ...
        fprintf(stderr, "[host] UnityMain nao concluiu shutdown em 10 s\n");
