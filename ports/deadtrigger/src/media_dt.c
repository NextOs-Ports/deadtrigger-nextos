 ...
 * original embedded MP4 is decoded to PulseAudio and /dev/fb0.
 */
#define _GNU_SOURCE
...
static void hooked_start_video(void *instance, const void *method) {
    ...
    char video[1024];
    if (extract_intro(video, sizeof video) != 0 ||
        access("/usr/bin/ffmpeg", X_OK) != 0 ||
        access("/usr/bin/pacat", X_OK) != 0 ||
        access("/dev/fb0", W_OK) != 0) {
        fprintf(stderr,
                "[video] backend NextOS indisponivel; encerrando a "
                "apresentacao pelo IntroVideoPlayer nativo\n");
    } else {
