...
def canonical_nxbootstrap_launcher(nxport, expected_version):
    ...
    try:
        return _NXBOOTSTRAP_GENERATOR.render_launcher(nxport)
    except (OSError, ValueError,
            _NXBOOTSTRAP_GENERATOR.ManifestError) as error:
        fail("cannot render the canonical nxbootstrap launcher: {}".format(
            error
        ))


def expected_nxextract_runtime_block(mode):
    if mode == "no":
        return "# NXExtract: disabled for this port (nxextract.mode=no)."
    requested = "1" if mode == "yes" else "0"
    auto_probe = ""
    if mode == "auto":
        auto_probe = """
...
unset nxprobe"""
    return """# NXExtract owner-data phase (BYO data): runs before the game, other process.
NXDIR=""
...
def expected_required_files_runtime_block(required_files):
    return """# Manifest-owned payload gate: extraction/prepare must finish before launch.
...
unset NXBOOTSTRAP_REQUIRED_FILES required_file required_path""" % shlex.quote(
        "\n".join(required_files))


def expected_executable_preflight(executable):
    return """# The executable is itself required and is touched before the owner-data phase.
NXBOOTSTRAP_EXECUTABLE=$(readlink -f "$GAMEDIR/%s" 2>/dev/null) || NXBOOTSTRAP_EXECUTABLE=""
case "$NXBOOTSTRAP_EXECUTABLE" in
  "$GAMEDIR"/*) ;;
  *) NXBOOTSTRAP_EXECUTABLE="" ;;
esac
if [ -z "$NXBOOTSTRAP_EXECUTABLE" ] || [ ! -f "$NXBOOTSTRAP_EXECUTABLE" ] || \\
   [ ! -s "$NXBOOTSTRAP_EXECUTABLE" ] || [ -L "$GAMEDIR/%s" ]; then
  echo "ERROR: executable is missing or unsafe: %s"
  nxbootstrap_finish
  exit 1
fi""" % (executable, executable, executable)
...
def verify_self_contained_wrapper(wrapper, wrapper_path, config, nx_arch,
    ...
    executable_preflight = expected_executable_preflight(nxport["executable"])
    ...
    nxextract_block = expected_nxextract_runtime_block(
    ...
    required_block = expected_required_files_runtime_block(
    ...
    if not (executable_preflight_index < nxextract_index < required_index <
            binary_char_index):
